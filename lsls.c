...
int main(int argc, char *argv[]){
        void read_directory(DIR *, char *); void fatal(char *);
        DIR *dir;char file[1024]; struct dirent *directory;
        if(argc == 1) strcpy(file, ".");else strcpy(file, argv[1]);
        if((dir = opendir(file)) == NULL) fatal("opendir");
        read_directory(dir, file);
}
void read_directory(DIR *dir, char *file){
        void fatal(char *);DIR *imsi_dir;struct dirent *directory;
        struct stat file_stat;char buff[1024];
